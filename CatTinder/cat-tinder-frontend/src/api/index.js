...
let editCats = function(id, cat) {
	console.log('cat id', id);
	console.log('cat', cat);

	return fetch(BASE + `/cats/${id}`, {// this would be equivalent to going to localhost:3000/cats in your browser. Do that - - what do you see?
		body: JSON.stringify(cat),  // <- we need to stringify the json for fetch
		headers: {  // <- We specify that we're sending JSON, and expect JSON back
			'Content-Type': 'application/json'
		},
